...
export const DEFAULT_DATA: Testimonial[] = [
  {
    name: "Sarah Chen",
    role: "CTO · Nexus Labs",
    initials: "SC",
    ...
    quote:
      "Completely transformed our product workflow. Worth every single penny.",
    r: 5,
  },
  {
    name: "Marcus Webb",
    ...
    quote:
      "The smoothest onboarding I've ever seen. Our entire team absolutely loves it.",
    r: 5,
  },
  {
    name: "Priya Sharma",
    role: "Head of Design · Orion",
    initials: "PS",
    g: ["#0EA5E9", "#06B6D4"],
    img: "https://i.pravatar.cc/80?img=5",
    quote:
      "Finally a tool that steps aside and lets us focus on building great things.",
    r: 5,
  },
  {
    name: "David Laurent",
    role: "VP Eng · Meridian",
    initials: "DL",
    g: ["#10B981", "#34D399"],
    img: "https://i.pravatar.cc/80?img=8",
    quote:
      "Cut our delivery time in half. ROI was immediate and completely undeniable.",
    r: 5,
  },
  {
    name: "Yuki Tanaka",
    role: "Product Lead · Arc",
    initials: "YT",
    g: ["#F59E0B", "#FBBF24"],
    img: "https://i.pravatar.cc/80?img=9",
    quote:
      "I recommend this to every founder I know. It's simply exceptional quality.",
    r: 5,
  },
  {
    name: "Alex Rivera",
    ...
    quote:
      "The quality and polish here is what we aspire to in our own products.",
    r: 5,
  },
  {
    name: "Emma Walsh",
    role: "Dir. Marketing · Pulse",
    initials: "EW",
    g: ["#F97316", "#EF4444"],
    img: "https://i.pravatar.cc/80?img=20",
    quote:
      "Conversion rate jumped 40% after switching. Truly remarkable results.",
    r: 5,
  },
  {
    name: "Raj Patel",
    role: "Architect · Vertex",
    initials: "RP",
    g: ["#6366F1", "#8B5CF6"],
    img: "https://i.pravatar.cc/15?img=15",
    quote:
      "Best investment our engineering team has made this year. No contest.",
    r: 5,
  },
  {
    name: "Nina Koch",
    ...
    quote:
      "Elegant, fast, reliable. Exactly what modern product teams deserve.",
    r: 5,
  },
  {
    name: "James Okafor",
    ...
    quote:
      "Shipped our MVP 3 weeks early because of this. Our investors noticed.",
    r: 5,
  },
  {
    name: "Lisa Park",
    ...
    quote:
      "The micro-interactions alone are worth it. Our users are absolutely delighted.",
    r: 5,
  },
  {
    name: "Tom Brennan",
    ...
    quote:
      "I've tried every alternative. Nothing comes close to this level of craft.",
    r: 5,
  },
];
...
const FloatingTestimonials: React.FC<FloatingTestimonialsProps> = ({
  ...
  useEffect(() => {
    ...
    circlesRef.current.forEach((c) => {
      ...
      if (c.el) {
        ...
        const body = c.el.querySelector(
          `.${styles["avatar-body"]}`,
        ) as HTMLElement;
        if (body) body.style.fontSize = Math.round(circleSize * 0.24) + "px";
      }
    });
    ...
    if (hero) {
      if (hasChildren) {
        ...
        gsap.to(hero, {
          ...
          duration: 0.35,
          ease: "power2.in",
          onComplete: () => hero.classList.add(styles.hidden),
        });
      }
    }
  }, [
  ...
  function showTip(circle: CircleNode) {
    ...
    if (tipStarsRef.current)
      tipStarsRef.current.innerHTML = Array(d.r)
        .fill(`<span class="${styles.star}">★</span>`)
        .join("");
  ...
  class Circle implements CircleNode {
    ...
    constructor(data: Testimonial, index: number) {
      ...
      const angle =
        (index / DATA.length) * Math.PI * 2 + (Math.random() - 0.5) * 0.4;
      const spread =
        Math.min(W.current, H.current) * (0.25 + Math.random() * 0.2);
    ...
    buildDOM() {
      ...
      <div class="${styles["avatar-body"]}" style="font-size:${Math.round(this.size * 0.24)}px;background:linear-gradient(135deg,${c1},${c2})">
    ...
    update(siblings: CircleNode[]) {
      ...
      this.vx += (Math.random() - 0.5) * 0.038 * cfg.current.randomness;
      this.vy += (Math.random() - 0.5) * 0.038 * cfg.current.randomness;
      ...
      if (cfg.current.wrapAround) {
        ...
        if (this.x > W.current - m)
          this.vx -= wall * (1 - (W.current - this.x) / m);
        if (this.y < m) this.vy += wall * (1 - this.y / m);
        if (this.y > H.current - m)
          this.vy -= wall * (1 - (H.current - this.y) / m);
      }

      if (cfg.current.hasChildren) {
        const cvR = 155;
        const cdx = this.x - W.current / 2,
          cdy = this.y - H.current / 2;
        const cd = Math.hypot(cdx, cdy);
        if (cd < cvR + this.r && cd > 0.1) {
  ...
  useEffect(() => {
    ...
    const heroEls = scene.querySelectorAll(
      `.${styles["hero-eyebrow"]}, .${styles["hero-title"]}, .${styles["hero-sub"]}, .${styles["hero-badge"]}`,
    );
    ...
    const tickHandler = () => {
      ...
      if (
        hov &&
        parseFloat(
          gsap.getProperty(getTooltip() as Element, "opacity") as string,
        ) > 0.4
      )
        posTip(hov);
    };
  ...
  function drawLines() {
    ...
    if (cfg.current.hasChildren) {
      ...
      circlesRef.current.forEach((c) => {
        ...
        const alpha = Math.max(
          0,
          0.12 - dist / (Math.min(W.current, H.current) * 1.4),
        );
    ...
    for (let i = 0; i < circles.length; i++) {
      for (let j = i + 1; j < circles.length; j++) {
        ...
        ctx.moveTo(a.x, a.y);
        ctx.lineTo(b.x, b.y);
        ctx.strokeStyle = `rgba(201,168,76,${(1 - d / 160) * 0.065})`;
        ctx.lineWidth = 0.6;
        ctx.stroke();
      }
    }
  ...
  return (
    <div className={styles.container}>
      <div className={styles.scene} ref={sceneRef}>
        ...
        <div
          className={`${styles["edge-fade"]} ${styles["ef-top"]} ${wrapAround ? styles.show : ""}`}
          style={{
            background: `linear-gradient(to bottom, ${bgColor}, transparent)`,
          }}
        ></div>
        <div
          className={`${styles["edge-fade"]} ${styles["ef-bottom"]} ${wrapAround ? styles.show : ""}`}
          style={{
            background: `linear-gradient(to top, ${bgColor}, transparent)`,
          }}
        ></div>
        <div
          className={`${styles["edge-fade"]} ${styles["ef-left"]} ${wrapAround ? styles.show : ""}`}
          style={{
            background: `linear-gradient(to right, ${bgColor}, transparent)`,
          }}
        ></div>
        <div
          className={`${styles["edge-fade"]} ${styles["ef-right"]} ${wrapAround ? styles.show : ""}`}
          style={{
        ...
        <div className={styles.hero} ref={heroRef}>
          ...
          <h1 className={styles["hero-title"]}>
            Loved by the
            <br />
            <em>world's best</em> teams
          </h1>
          <p className={styles["hero-sub"]}>
            Join companies transforming how they build and ship products.
          </p>
          <div className={styles["hero-badge"]}>
            <span className={styles["pulse-dot"]}></span>12,400+ happy customers
          </div>
        </div>
      </div>
      <div className={styles.tooltip} ref={tooltipRef}>
        <div className={styles.tipShell}>
          <div className={styles.tipTop}>
            ...
            <div>
              <div className={styles.tipName} ref={tipNameRef}></div>
              <div className={styles.tipRole} ref={tipRoleRef}></div>
            </div>
          </div>
